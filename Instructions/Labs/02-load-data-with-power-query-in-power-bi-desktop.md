---
lab:
  title: Caricare i dati trasformati in Power BI Desktop
  module: 'Clean, Transform, and Load Data in Power BI'
---

# Caricare i dati trasformati in Power BI Desktop
...
1. Per aprire la finestra **Editor di Power Query**, nella scheda della barra multifunzione **Home**, nel gruppo **Query** selezionare l'icona **Trasforma dati**.
     ...
     ![Trasformare i dati sulla barra multifunzione Home](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image10.png)
...
1. Per individuare una colonna specifica, nella scheda **Home** della barra multifunzione selezionare la freccia a discesa **Gestisci colonne** , selezionare la freccia in giù **Scegli colonne** e quindi selezionare **Vai a Colonna**.
    ...
    *Passare a Colonna è una funzionalità utile con molte colonne. In caso contrario, è possibile scorrere orizzontalmente le colonne.*
     ...
     ![Gestire le colonne > Scegliere colonne > Vai alla colonna](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image13.png)
...
1. Individuare la colonna **SalesPersonFlag** , quindi filtrare la colonna per selezionare solo Salespeople (ovvero **TRUE**) e fare clic su **OK**.

1. Nel riquadro **Impostazioni query**, nell'elenco **Passaggi applicati**, notare l'aggiunta del passaggio **Filtrate righe**.

    *Ogni trasformazione creata crea risultati in un'altra logica di passaggio. È possibile modificare o eliminare i passaggi. È anche possibile selezionare un passaggio per visualizzare in anteprima i risultati della query in tale fase della trasformazione di query.*
...
1. Nell'elenco **Passaggi applicati** si noti l'aggiunta di un altro passaggio della query.
     ...
     ![Passaggio di altre colonne rimosse](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image21.png)
...
1. Per rinominare la colonna **EmployeeNationalIDAlternateKey** , fare doppio clic sull'intestazione di colonna **EmployeeNationalIDAlternateKey** e sostituire il testo con **EmployeeID** e quindi premere **INVIO**.
...
*Importante: quando sono già state fornite istruzioni dettagliate, i passaggi del lab forniscono istruzioni più concise. Se sono necessarie le istruzioni dettagliate, è possibile fare riferimento ai passaggi delle attività precedenti.*

1. Selezionare la query **DimProduct** e rinominare la query su **Product**.

1. Individuare la colonna **FinishedGoodsFlag** e quindi filtrare la colonna per recuperare i prodotti finiti, ovvero TRUE.

1. Rimuovere tutte le colonne, **tranne** quanto segue:
...
1. Nell'intestazione **di colonna DimProductSubcategory** , a destra del nome della colonna, selezionare il pulsante espandi.
    ...
    ![Icona di espansione colonna](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image31.png)
...
In questa attività verrà configurata la query **Rivenditore** .

1. Selezionare la query **DimReseller** e rinominare **rivenditore**.
...
1. Nell'intestazione della colonna **Tipo di business** selezionare la freccia giù e quindi esaminare i valori di colonna distinti e notare entrambi i valori **Warehouse** e **Ware House**.
...
1. Selezionare la query **DimSalesTerritory** e rinominare la query in **Area**.
...
1. Selezionare la query **FactResellerSales** e rinominarla in **Sales**.
...
1. Rimuovere tutte le colonne, **tranne** quanto segue:
    ...
    - DimProduct
        ...
        *Nota: è possibile ricordare nel lab **Prepare Data in Power BI Desktop** lab che una piccola percentuale di righe **FactResellerSales** aveva valori **TotalProductCost** mancanti. La colonna **DimProduct** è stata inclusa per recuperare la colonna di costo standard del prodotto per facilitare la correzione dei valori mancanti.*

1. Espandere la colonna **DimProduct**, deselezionare tutte le colonne e quindi includere la colonna **StandardCost**.

1. Per creare una colonna personalizzata, nella scheda Aggiungi barra multifunzione **Colonna** all'interno del gruppo **Generale** selezionare **Colonna personalizzata**.
...
1. Nella casella **Formula colonna personalizzata** immettere l'espressione seguente (dopo il simbolo di uguale).
    ...
    - *Questa espressione verifica se il valore **TotalProductCost** non è presente. Se mancante, genera un valore moltiplicando il valore **OrderQuantity** in base al valore **StandardCost** ; in caso contrario, usa il valore **TotalProductCost** esistente.*
...
1. Per modificare il tipo di dati della colonna, nell'intestazione **Della colonna Quantity** , a sinistra del nome della colonna selezionare l'icona **1.2** e quindi selezionare **Numero intero**.
...
1. Modificare i tipi di dati delle tre colonne seguenti in **Numero decimale fisso**.
    ...
    *Il tipo di dati numerico decimale fisso consente 19 cifre e consente una maggiore precisione per evitare errori di arrotondamento. È importante usare il tipo di numero decimale fisso per i valori finanziari o i tassi (ad esempio tassi di cambio).*
...
1. Selezionare la query **ResellerSalesTargets** e rinominare **target**.
...
1. Per moltiplicare i valori **di destinazione** per 1000, selezionare l'intestazione **di colonna Target** e quindi nella scheda **Trasforma** della barra multifunzione, dall'interno del gruppo **Colonna numero** selezionare **Standard** e **quindi moltiplicare**.
...
1. Quando viene visualizzata la finestra **Livelli di privacy** , per ognuna delle due origini dati, nell'elenco a discesa corrispondente selezionare **Organizzazione**, quindi **Salva**.
    ...
    *I livelli di privacy possono essere configurati per l'origine dati per determinare se i dati possono essere condivisi tra origini. Se necessario, l'impostazione di ogni origine dati come **organizzazione** consente di condividere i dati. Le origini dati private non possono mai essere condivise con altre origini dati. Non significa che i dati privati non possono essere condivisi; significa che il motore di Power Query non può condividere dati tra le origini.*

     ![Immagine 5691](Linked_image_Files/02-load-data-with-power-query-in-power-bi-desktop_image74.png)

1. Nella finestra **Merge** usare il **tipo di join** predefinito, mantenendo la selezione di Left Outer e selezionare **OK**.
...
1. Nel riquadro **Dati** ,che si trova a destra, si notino le sette tabelle caricate nel modello di dati.
