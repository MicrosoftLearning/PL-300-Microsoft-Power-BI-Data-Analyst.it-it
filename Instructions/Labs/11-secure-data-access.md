...
In questo lab verrà applicata la sicurezza a livello di riga per garantire che un venditore possa analizzare solo i dati delle vendite per le aree assegnate. Si apprenderà come:

- Implementare la sicurezza dinamica a livello di riga in Power BI.
- Creare e testare un ruolo usando USERPRINCIPALNAME().
...
1. Per assegnare un filtro, selezionare la **tabella Salesperson (Performance)** e quindi selezionare **Passa all'editor** DAX nella **sezione Regole** .
...
1. Si noti il banner rosso sopra la pagina del report, che descrive il contesto di sicurezza del test.
...
1. Per interrompere il test, sul lato destro del banner rosso selezionare **Arresta visualizzazione**.
