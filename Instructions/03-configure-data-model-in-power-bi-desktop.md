...
1. Per creare un oggetto visivo tabella, nel riquadro **Dati** , dall'interno della tabella **Product** selezionare il campo **Categoria** .

1. Per aggiungere un'altra colonna alla tabella, nel riquadro **Dati** selezionare il campo **Sales Sales\|**.
...
1. In Visualizzazione modello, nel riquadro **Dati** , se necessario, espandere la tabella **Product** per visualizzare tutti i campi.
...
1. Nel riquadro **Proprietà** (a sinistra del riquadro **Dati** ), nella casella **Nome** sostituire il testo con **Prodotti**.
...
1. Nel riquadro **Dati** notare la gerarchia **Prodotti** .
...
1. Nel riquadro **Dati** notare che le due colonne sono ora all'interno di una cartella.
...
1. Nella casella **Descrizione** del riquadro **Proprietà** immettere: *In base al costo standard*.
...
In questa attività si aggiorneranno più colonne usando singoli aggiornamenti in blocco. Questo approccio verrà usato per nascondere le colonne e formattare i valori delle colonne.

1. Nel riquadro **Dati** selezionare la colonna **ProductKey\|**.
...
1. Nel riquadro **Proprietà** impostare l'interruttore della proprietà **È nascosto** su **Sì**.
    ...
    *Si userà **SalesOrderNumber** in un calcolo nel lab **Creare calcoli DAX in Power BI Desktop** lab.*
...
1. Espandere il campo **Sales \| OrderDate** notando che viene visualizzata una gerarchia di date.
    ...
    *Il campo **Target \| TargetMonth** fornisce una gerarchia simile. Queste gerarchie non sono state create dall'utente. Sono stati creati automaticamente. Tuttavia, c'è un problema. L'anno finanziario Adventure Works inizia il 1° luglio di ogni anno. Tuttavia, in queste gerarchie di date create automaticamente, l'anno della gerarchia di date inizia il 1° gennaio di ogni anno.*

     ![Immagine 359](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image40.png)

 A questo punto si disattiva questo comportamento automatico. Nel lab **Creare calcoli DAX in Power BI Desktop** si userà DAX per creare una tabella di date e configurarla per definire il calendario di Adventure Works.

1. Per disattivare l'ora automatica/data, passare a **File > Opzioni e impostazioni > Opzioni > gruppo file corrente** e selezionare **Caricamento dati**.
...
1. Nel riquadro **Dati** notare che le gerarchie di date non sono più disponibili.
...
*Una misura rapida crea automaticamente la formula di calcolo. Sono facili e veloci da creare per calcoli semplici e comuni. Si creeranno misure senza usare questo strumento nel lab **Creare calcoli DAX in Power BI Desktop**.*
